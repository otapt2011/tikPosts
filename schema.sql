...
CREATE VIEW IF NOT EXISTS vw_following AS SELECT * FROM profiles WHERE is_following = 1;
CREATE VIEW IF NOT EXISTS vw_followers AS SELECT * FROM profiles WHERE is_follower = 1;
CREATE VIEW IF NOT EXISTS vw_friends AS SELECT * FROM profiles WHERE is_following = 1 AND is_follower = 1;
CREATE VIEW IF NOT EXISTS vw_blocked AS SELECT * FROM profiles WHERE is_blocked = 1;
CREATE VIEW IF NOT EXISTS vw_following_only AS SELECT * FROM profiles WHERE is_following = 1 AND is_follower = 0;
CREATE VIEW IF NOT EXISTS vw_followers_only AS SELECT * FROM profiles WHERE is_follower = 1 AND is_following = 0;
CREATE VIEW IF NOT EXISTS vw_blocked_with_follow AS SELECT * FROM profiles WHERE is_blocked = 1 AND is_following = 1;
...
CREATE VIEW IF NOT EXISTS vw_verified_profiles AS SELECT * FROM profiles WHERE verified = 1;
...
VALUES
(NULL, 'All Following (current owner)', 'SELECT * FROM profiles WHERE owner_username = ? AND is_following = 1 ORDER BY username;'),
(NULL, 'All Followers (mutual + imported)', 'SELECT * FROM profiles WHERE owner_username = ? AND is_follower = 1 ORDER BY username;'),
...
(NULL, 'Follower Growth Opportunity (Following me but I don''t follow back)', 'SELECT username, displayName, followerCount FROM profiles WHERE owner_username = ? AND is_follower = 1 AND is_following = 0;'),
